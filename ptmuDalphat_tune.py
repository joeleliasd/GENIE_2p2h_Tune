...
binStructuredalphat = [8, 14, 14, 14, 13, 12, 10, 7, 6, 4, 3]
...
def scale(into):
    ...
    for i in range(histX):
        for f in range(binStructuredalphat[i]):
...
dalphat_nBins=np.array(eval(pd.read_csv("h_dalphat_ptmu_binning.tsv", header=None, sep='\t')[0][0]))
xbins_dalphat_boundaries,xbins_dalphat,xbins_dalphat_xerr=xbin_maker(dalphat_nBins,"h_dalphat_ptmu_boundaries.tsv")

dalphat_ptmu_all,dalphat_ptmu=import_data("h_dalphat_ptmu_data_result.tsv")
MnvData=BinInScheme_Norm(dalphat_ptmu,dalphat_nBins,xbins_dalphat_boundaries,5)
MnvMC=scale(read_to_jagged_array('ptmu_dalphat_MC.tsv'))#mc of all diff xs

del dalphat_ptmu[-1]

print('Dalphat length: ')
print(len(dalphat_ptmu))
print(dalphat_nBins)
print()
...
error,covmtx=cov_prune('dalphat_ptmu_cov.tsv')
...
ax.set_title('∆dalphat:∆alphat Covariance')
ax.set_aspect('equal')
...
div1 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div1MQ_UR.tsv'))
div2 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div2MQ_UR.tsv'))
div3 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div3MQ_UR.tsv'))
div4 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div4MQ_UR.tsv'))
div5 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div5MQ_UR.tsv'))
div6 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div6MQ_UR.tsv'))
div7 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div7MQ_UR.tsv'))
div8 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div8MQ_UR.tsv'))
div9 = scale(read_to_jagged_array('h_ptmu_dalphat_2p2h_Div9MQ_UR.tsv'))
...
Mnvpion=scale(read_to_jagged_array('ptmu_dalphat_mc_pion.tsv'))#pion component of mc
Mnvqe=scale(read_to_jagged_array('ptmu_dalphat_mc_qe.tsv'))#qe component of mc
Mnv2p2h=scale(read_to_jagged_array('ptmu_dalphat_mc_2p2h.tsv')) #2p2h component of mc
Mnv2p2h_2=Mnv2p2h
# Loop through each entry in the first dimension
for i in range(len(binStructuredalphat)):
    # Loop through each entry in the second dimension
    for j in range(binStructuredalphat[i]):
...
Mnvdatano2p2h = [ [0]*length for length in binStructuredalphat]
...
Mnvmcno2p2h=[ [0]*length for length in binStructuredalphat]
...
Mnv2p2hdata=[ [0]*length for length in binStructuredalphat]
...
NUISANCE2p2hb=[ [0]*length for length in binStructuredalphat]

for i in range(numberOfDivisions):
    NUISANCE2p2hb = (div1 + div2 + div3 + div4 + div5 + div6 + div7 + div8 + div9)

# Perform element-wise division
Mnv2p2hFraction=[ [0]*length for length in binStructuredalphat]
# Loop through each entry in the first dimension
for i in range(len(binStructuredalphat)):
    # Loop through each entry in the second dimension
    for j in range(binStructuredalphat[i]):
...
if lmda != 0:
    ...
    fitted2p2hb = [ [0]*length for length in binStructuredalphat]
    for i in range(len(binStructuredalphat)):
        row_length = binStructuredalphat[i] - 1
    ...
    for i in range(histX):
        for f in range(binStructuredalphat[i]):
            fitted2p2hb[i][f]=weightedmerged2p2hlistb[i+f]/BinNorm[i][j]
    NUISANCE2p2hb=np.array_split(merged2p2hlistb@ones,histX)

noTuneBins=[ [0]*length for length in binStructuredalphat]

for f in range(histX):
    for i in range(binStructuredalphat[f]):
        if Mnv2p2h[f][i] == 0:
            noTuneBins[f][i] = 0
...
newFit=[ [0]*length for length in binStructuredalphat]
for f in range(histX):
    for i in range(binStructuredalphat[f]):
        newFit[f][i] = fitted2p2hb[f][i] - Mnv2p2h[f][i] + MnvMC[f][i]
...
for i in range(histX):
    ...
    for f in range(len(newFit[i])):
        ...
        plt.plot(xdalphat[i][f], 0, marker='o', color=color)

    # Plot data
    p1, = plt.plot(xdalphat[i], (newFit)[i], color='r', label="Tune")
    p2, = plt.plot(xdalphat[i], MnvMC[i], color='r', linestyle='dashed', label="Untuned Total Prediction")
    p3 = plt.errorbar(xdalphat[i], MnvData[i], yerr=error[i], fmt="o", color="k", capsize=5, markersize=2.9, label="Data")
    p4, = plt.plot(xdalphat[i], (Mnv2p2h_2)[i], color='b', label="2p2h")
    p5, = plt.plot(xdalphat[i], (Mnvqe)[i], color='purple', label="QE")
    p6, = plt.plot(xdalphat[i], (Mnvpion)[i], color='cyan', label="Pion")
    ...
    plt.title(f"ptmu vs dalphat - ptmu bin {i+1}")
    plt.xlabel(r'$\Delta \alpha_{T}$')

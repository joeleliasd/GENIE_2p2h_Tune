...
binStructureDphit = [7, 18, 19, 19, 21, 21, 21, 21, 21, 21, 16]
...
def scale(into):
    ...
    for i in range(histX):
        for f in range(binStructureDphit[i]):
...
dphit_nBins=np.array(eval(pd.read_csv("h_dphit_ptmu_binning.tsv", header=None, sep='\t')[0][0]))
xbins_dphit_boundaries,xbins_dphit,xbins_dphit_xerr=xbin_maker(dphit_nBins,"h_dphit_ptmu_boundaries.tsv")

dphit_ptmu_all,dphit_ptmu=import_data("h_dphit_ptmu_data_result.tsv")
MnvData=BinInScheme_Norm(dphit_ptmu,dphit_nBins,xbins_dphit_boundaries,5)
MnvMC=scale(read_to_jagged_array('ptmu_dphit_MC.tsv'))#mc of all diff xs
...
error,covmtx=cov_prune('dphit_ptmu_cov.tsv')
...
#ax.set_xlim3d(0,0.4
#x.set_xlabel('∆phit', fontsize = 10)
#x.set_ylabel('∆alphat', fontsize = 10)
ax.set_title('∆dphit:∆alphat Covariance')
ax.set_aspect('equal')
...
div1 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div1MQ_UR.tsv'))
div2 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div2MQ_UR.tsv'))
div3 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div3MQ_UR.tsv'))
div4 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div4MQ_UR.tsv'))
div5 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div5MQ_UR.tsv'))
div6 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div6MQ_UR.tsv'))
div7 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div7MQ_UR.tsv'))
div8 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div8MQ_UR.tsv'))
div9 = scale(read_to_jagged_array('h_ptmu_dphit_2p2h_Div9MQ_UR.tsv'))
...
# 
# for ax in axsb.flat:
#     ax.set(xlabel='∆phit', ylabel='∆alphat')
# 
...
# #ax.set_xlim3d(0,0.4
# ax.set_xlabel('∆phit', fontsize = 10)
# ax.set_ylabel('∆alphat', fontsize = 10)
# ax.set_title('All double diff xs')
# plt.pcolormesh(Allplot, cmap='coolwarm')
# plt.colorbar(location='bottom')
# plt.show()

Mnvpion=scale(read_to_jagged_array('ptmu_dphit_mc_pion.tsv'))#pion component of mc
Mnvqe=scale(read_to_jagged_array('ptmu_dphit_mc_qe.tsv'))#qe component of mc
Mnv2p2h=scale(read_to_jagged_array('ptmu_dphit_mc_2p2h.tsv')) #2p2h component of mc

Mnv2p2h_2=Mnv2p2h
# Loop through each entry in the first dimension
for i in range(len(binStructureDphit)):
    # Loop through each entry in the second dimension
    for j in range(binStructureDphit[i]):
        if (Mnv2p2h[i][j] / MnvMC[i][j]) < .2:
            Mnv2p2h[i][j] = 0

print('\nMnv2p2h', Mnv2p2h,'\n')

# Initialize the result array as a jagged array
Mnvdatano2p2h = [ [0]*length for length in binStructureDphit]
...
Mnvmcno2p2h=[ [0]*length for length in binStructureDphit]
...
Mnv2p2hdata=[ [0]*length for length in binStructureDphit]
...
NUISANCE2p2hb=[ [0]*length for length in binStructureDphit]

for i in range(numberOfDivisions):
    NUISANCE2p2hb = (div1 + div2 + div3 + div4 + div5 + div6 + div7 + div8 + div9)

# Perform element-wise division
Mnv2p2hFraction=[ [0]*length for length in binStructureDphit]
# Loop through each entry in the first dimension
for i in range(len(binStructureDphit)):
    # Loop through each entry in the second dimension
    for j in range(binStructureDphit[i]):
        # Directly subtract and assign the result
        Mnv2p2hFraction[i][j] = Mnv2p2h[i][j] / MnvMC[i][j]
...
def Chi_squaredb(lst):
    chi2b=((Mnv2p2hdataflat-(merged2p2hlistb@exp_fit_pramsb(lst)))@invcovmtx@(Mnv2p2hdataflat-(merged2p2hlistb@exp_fit_pramsb(lst))))
    return chi2b
...
if lmda != 0:
    ...
    while not FitSuccess:
        m = Minuit.from_array_func(Chi_squaredb, initial_guesses, error=0.1, errordef=1)
        m.migrad()
        FitSuccess = m.valid
        binWeightsb = np.array(m.values) 
    ...
    count=0
    chi_array=np.zeros(3)
    binWeightsb = np.zeros(numberOfDivisions)
    FitSuccess = False
    while not FitSuccess:
        print('Bin Weights: ', exp_fit_pramsb(binWeightsb))
        chi_array[count]=Chi_squaredb(binWeightsb)
        m = Minuit(Chi_squaredb, binWeightsb)
        m.migrad()
        FitSuccess = m.valid
        binWeightsb = np.array(m.values)
        ...
        if count>2:
            break;
    print('Chi values during tune: ', chi_array)
    plt.plot(chi_array)
    plt.show()
    zeros = np.zeros(numberOfDivisions)
    ones = np.ones(numberOfDivisions)
    print('Chi: ', Chi_squaredb(binWeightsb))
    print()
    print('Chi untuned: ', Chi_squaredb(zeros))
    print()
    print('Bin weights: ')
    weightsb=exp_fit_pramsb(binWeightsb)
    print(weightsb)
    ...
    # Make fitted2p2hb
    fitted2p2hb = [ [0]*length for length in binStructureDphit]
    for i in range(len(binStructureDphit)):
        row_length = binStructureDphit[i] - 1
...
#Bins not present in the tune being weighted and re-addded

noTuneBins=[ [0]*length for length in binStructureDphit]

for f in range(histX):
    for i in range(binStructureDphit[f]):
        if Mnv2p2h[f][i] == 0:
            noTuneBins[f][i] = 0
...
low2p2hWeightedBins=[ [0]*length for length in binStructureDphit]

for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div1[f][i]*weightsb[0]
     
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div2[f][i]*weightsb[1]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div3[f][i]*weightsb[2]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div4[f][i]*weightsb[3]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div5[f][i]*weightsb[4]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div6[f][i]*weightsb[5]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div7[f][i]*weightsb[6]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div8[f][i]*weightsb[7]
            
for f in range(histX):
    for i in range(binStructureDphit[f]):
        if noTuneBins[f][i]==1:
            low2p2hWeightedBins[f][i]=low2p2hWeightedBins[f][i]+div9[f][i]*weightsb[8]

#Bin Normalize components

newFit=[ [0]*length for length in binStructureDphit]
for f in range(histX):
    for i in range(binStructureDphit[f]):
        newFit[f][i] = (fitted2p2hb[f][i] - Mnv2p2h[f][i] + MnvMC[f][i] + low2p2hWeightedBins[f][i])/BinNorm[f][i]
...
BinNormFlat = np.concatenate(BinNorm)
for i in range(205):
    error[i]=error[i]/BinNormFlat[i]
        
# =============================================================================
# for f in range(histX):
#     for i in range(binStructureDphit[f]):
#         error[i+f] = error[i]/BinNorm[f][i]
# =============================================================================
...
for i in range(histX):
    ...
    for f in range(len(newFit[i])):
        ...
        plt.plot(xDphit[i][f], 0, marker='o', color=color)
    
    # Plot data
    p1, = plt.plot(xDphit[i], (newFit)[i], color='r', label="Tune")
    p2, = plt.plot(xDphit[i], MnvMC[i], color='r', linestyle='dashed', label="Old Prediction")
    p3 = plt.errorbar(xDphit[i], MnvData[i], yerr=error[i], fmt="o", color="k", capsize=5, markersize=2.9, label="Data")
    p4, = plt.plot(xDphit[i], (Mnv2p2h_2)[i], color='b', label="2p2h")
    p5, = plt.plot(xDphit[i], Mnvqe[i], color='purple', label="QE")
    p6, = plt.plot(xDphit[i], (Mnvpion)[i], color='cyan', label="Pion")
    ...
    plt.title(f"ptmu vs dphit - ptmu bin {i+1}")
    plt.xlabel(r'$\Delta \phi_{T}$')
    plt.ylabel("Differential Cross Section")
    plt.legend(handles=[tuned_patch, untuned_patch, p1, p2, p3, p4, p5, p6], labels=["Normal Bin", "Low 2p2h Bin", "Tuned Total", "Untuned Total", "Total Data", "2p2h", "QE", "π"], fontsize=8)
    plt.show()